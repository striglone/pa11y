...
module.exports.actions = [
	...
	// Action to disable Content Security Policy
	// E.g. "disable csp"
	// E.g. "disable content security policy"
	{
		name: 'disable-csp',
		match: /^disable( content security policy|csp)$/i,
		run: async (browser, page, options, matches) => {
			try {
				await page.evaluate(() => {
					...
					// Create permissive CSP that allows SSO connections
					const meta = document.createElement('meta');
					meta.httpEquiv = 'Content-Security-Policy';
					meta.content = `default-src 'self' 'unsafe-inline' 'unsafe-eval' data: blob:; connect-src 'self' http://${currentHost}:* https://${currentHost}:* http://localhost:* https://localhost:* ws://${currentHost}:* wss://${currentHost}:* ws://localhost:* wss://localhost:*;`;
					document.head.appendChild(meta);
					
					console.log('CSP disabled for testing - allowing SSO connections');
				});
			} catch (error) {
				throw new Error(`Failed action: Could not disable CSP`);
			}
		}
	},
	...
	// E.g. "set field #username to example"
	{
		...
		run: async (browser, page, options, matches) => {
			...
			try {
				await page.evaluate(async (targetSelector, desiredValue) => {
					const target = document.querySelector(targetSelector);
					if (!target) {
						return Promise.reject(new Error('No element found'));
					}
					
					// Helper function to create more realistic events
					function createRealisticEvent(type, options = {}) {
						...
						// Make the event appear more trusted
					...
					function createRealisticKeyboardEvent(type, options = {}) {
					...
					function createRealisticInputEvent(type, options = {}) {
					...
					target.focus();
					await wait(10); // Small delay to ensure focus is processed
					
					// Dispatch focus event
					target.dispatchEvent(createRealisticEvent('focus'));
					await wait(10);
					
					// Clear existing value first
					const currentValue = target.value;
					if (currentValue) {
						// Dispatch beforeinput for clearing
						target.dispatchEvent(createRealisticInputEvent('beforeinput', {
						...
						target.dispatchEvent(createRealisticInputEvent('input', {
							inputType: 'deleteContentBackward',
							data: null
						}));
						await wait(10);
					...
					// Dispatch keyboard events and set value character by character
					for (let i = 0; i < desiredValue.length; i++) {
						const char = desiredValue[i];
						...
						// Keydown event
						target.dispatchEvent(createRealisticKeyboardEvent('keydown', {
							key: char,
						...
						// Keypress event
						target.dispatchEvent(createRealisticKeyboardEvent('keypress', {
							key: char,
							code: `Key${char.toUpperCase()}`,
							keyCode: keyCode,
							which: keyCode,
						...
						const {set: prototypeValueSetter} =
							Object.getOwnPropertyDescriptor(prototype, 'value') || {};
						if (prototypeValueSetter) {
							prototypeValueSetter.call(target, newValue);
						} else {
						...
						// Keyup event - this is the critical one for your use case
						target.dispatchEvent(createRealisticKeyboardEvent('keyup', {
							key: char,
							code: `Key${char.toUpperCase()}`,
							keyCode: keyCode,
							which: keyCode,
						...
						await wait(10); // Longer delay after keyup to ensure it's processed
					}
					
					// Dispatch change event
					target.dispatchEvent(createRealisticEvent('change'));
					await wait(10);
					
					// Dispatch blur event
					target.dispatchEvent(createRealisticEvent('blur'));
					await wait(10);
			...
			try {
				await page.evaluate(targetSelector => {
					...
					if (currentValue) {
						...
						// Simulate Ctrl+A (select all) - keydown
						...
						// Simulate Ctrl+A (select all) - keypress
						...
						// Simulate Ctrl+A (select all) - keyup
